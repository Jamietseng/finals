...
bgr_1 = pygame.image.load('C:\\Users\\User\\Desktop\\A+雕課師\\bgr-1.png')  # 引入課表00:00-08:00
bgr_1.convert_alpha()  # 畫入課表00:00-08:00圖檔
bgr_1 = pygame.transform.smoothscale(bgr_1, (1360, 765))  # 長寬設成1360*765（視窗大小）

bgr_2 = pygame.image.load('C:\\Users\\User\\Desktop\\A+雕課師\\bgr-2.png')  # 引入課表08:00-16:00
bgr_2.convert_alpha()  # 畫入課表08:00-16:00圖檔
bgr_2 = pygame.transform.smoothscale(bgr_2, (1360, 765))  # 長寬設成1360*765（視窗大小）

bgr_3 = pygame.image.load('C:\\Users\\User\\Desktop\\A+雕課師\\bgr-3.png')  # 引入課表16:00-00:00
bgr_3.convert_alpha()  # 畫入課表16:00-00:00圖檔
...
bglb_4 = pygame.image.load('C:\\Users\\User\\Desktop\\A+雕課師\\bglb-4.png')  # 引入藍色方塊（點選的時段呈藍色）
bglb_4.convert_alpha()  # 畫入藍色方塊圖檔
bglb_4 = pygame.transform.smoothscale(bglb_4, (85, 51))  # 長寬設成85*51

bglb_5 = pygame.image.load('C:\\Users\\User\\Desktop\\A+雕課師\\bglb-5.png')  # 引入深灰色方塊（滑過的時段呈深灰色）
bglb_5.convert_alpha()  # 畫入深灰色方塊圖檔
...
def J2_r():
    ...
    if playJ2r == 0:
        ...
        for b5 in range(lecture):
            ...
            if Ssort_dict[orderS[0][1]][8] >= 100 or Ssort_dict[orderS[0][1]][0] < 0:
                Ssort_dict[orderS[0][1]][0] = 0
            Csort_dict[orderC[-1][1]][6] -= 1
            if Csort_dict[orderC[-1][1]][6] == 0:
                del Csort_dict[orderC[-1][1]]
        for b6 in Csort_dict:
            breakP = []
            for b7 in range(7):
                accu = 0
                for b8 in range(3):
            ...
            breakP.reverse()
            for b10 in breakP:
                if b10[0] < 2 * Csort_dict[b6][6]:
                    if b10[0] >= Csort_dict[b6][6]:
                        for b11 in range(Csort_dict[b6][6]):
                            if -8 <= -1 * b10[2] + b11 < 0:
                                print_list[-1 * b10[1] - 1][8 - b10[2] + b11][-1 * b10[3]][0] = 1
                                print_list[-1 * b10[1] - 1][8 - b10[2] + b11][-1 * b10[3]][1] = b6
                                Ssort_dict[b6][8] += Csort_dict[b6][0] / Csort_dict[b6][1]
                            elif -1 * b10[2] + b11 < -8:
                        ...
                        for b12 in range(b10[0]):
                            if -8 <= -1 * b10[2] + b12 < 0:
                                ...
                                Ssort_dict[b6][8] += Csort_dict[b6][0] / Csort_dict[b6][1]
                        Csort_dict[b6][6] -= b10[0]
                else:
                    continue
            for b13 in range(Csort_dict[b6][6]):
                orderS = sorted(zip(Ssort_dict.values(), Ssort_dict.keys()), reverse=True)
                Ssort_dict[orderS[0][1]][8] += Ssort_dict[orderS[0][1]][0] / Ssort_dict[orderS[0][1]][1]
                Ssort_dict[orderS[0][1]][0] -= (0.1 + Ssort_dict[orderS[0][1]][7] * Ssort_dict[orderS[0][1]][6]) * Ssort_dict[orderS[0][1]][1]
                Ssort_dict[orderS[0][1]][6] += 1
        ...
        for b15 in Ssort_dict:
            if Ssort_dict[b15][0] == 0:
                Ssort_dict[b15][0] = (Ssort_dict[b15][9] - (0.1 * Ssort_dict[b15][6] + Ssort_dict[b15][7] * (Ssort_dict[b15][6] - 1))) * Ssort_dict[b15][1]
        checkDict = dict()
        for b21 in Ssort_dict:
            checkDict[b21] = Ssort_dict[b21][8]
        check_list = sorted(zip(checkDict.values(), checkDict.keys()), reverse=True)
        for b16 in check_list:
            while Ssort_dict[b16[1]][8] < 60:
                orderS = sorted(zip(Ssort_dict.values(), Ssort_dict.keys()), reverse=True)
                change = -1
                while True:
                    if orderS[change][0][8] - orderS[change][0][0] / orderS[change][0][1] - (0.1 + (orderS[change][0][6] - 1) * orderS[change][0][7]) < 60:
                        if -1 * change == len(Ssort_dict):
                            break
                        change -= 1
                        continue
                    else:
                        if Ssort_dict[orderS[change][1]][6] == 0:
                            change -= 1
                            continue
                        Ssort_dict[orderS[change][1]][8] -= Ssort_dict[orderS[change][1]][0] / Ssort_dict[orderS[change][1]][1] - (0.1 + (Ssort_dict[orderS[change][1]][7] - 1) * Ssort_dict[orderS[change][1]][6])
                        Ssort_dict[orderS[change][1]][0] += (0.1 + (Ssort_dict[orderS[change][1]][6] - 1) * Ssort_dict[orderS[change][1]][7]) * Ssort_dict[orderS[change][1]][1]
                        Ssort_dict[orderS[change][1]][6] -= 1
        ...
        for b17 in subj:
            ...
            if Ssort_dict[b17][6] == 0:
                del Ssort_dict[b17]
        put_in = sorted(zip(Ssort_dict.keys(), Ssort_dict.values()))
        for b18 in range(7):
            for b19 in range(3):
                for b20 in range(8):
                    if bglb_list[b19][b20][b18][1] != -1 and bglb_list[b19][b20][b18][1] != '讀書' and subj[bglb_list[b19][b20][b18][1]][6] == '是':
                        print_list[b19][b20][b18][0] = 1
                        print_list[b19][b20][b18][1] = bglb_list[b19][b20][b18][1]
                    elif bglb_list[b19][b20][b18][1] == '讀書' or (bglb_list[b19][b20][b18][1] != -1 and bglb_list[b19][b20][b18][1] != '讀書' and print_list[b19][b20][b18][1] == -1):
                        print_list[b19][b20][b18][0] = 2
                        try:
                            print_list[b19][b20][b18][1] = put_in[0][0]
                            put_in[0][1][6] -= 1
                            if put_in[0][1][6] == 0:
                                put_in.remove(put_in[0])
                        except:
                            print_list[b19][b20][b18][1] = '讀書'
        playJ2r = 1
    else:
        mouses = pygame.mouse.get_pos()
        buttons = pygame.mouse.get_pressed()
        ...
        for b21 in range(7):
            pred = fontObj.render('{:^5s}'.format(str(orid.month) + '/' + str(orid.day)), True, (0, 0, 0))  # 日期
            prew = fontObj.render('{:^5s}'.format('（' + week[orid.weekday()] + '）'), True, (0, 0, 0))  # 星期幾
            x = datetime.timedelta(days=b21 + 1)
            orid = start + x  # 加一天
            window.blit(pred, (482 + 113 * b21, 81))
            window.blit(prew, (468 + 113 * b21, 110))
        ...
        for b22 in range(8):
            for b23 in range(7):
                if print_list[bgr_schedule - 1][b22][b23][0] == 0:  # 狀態零為變淺灰色
                    window.blit(bglb_3, (465 + print_x, 145 + print_y))
                elif print_list[bgr_schedule - 1][b22][b23][0] == 1:  # 狀態一為變綠色+顯示課程名稱
                    window.blit(bglb_4, (465 + print_x, 145 + print_y))
                    prename = fontObj.render(print_list[bgr_schedule - 1][b22][b23][1] if len(print_list[bgr_schedule - 1][b22][b23][1]) <= 3 else print_list[bgr_schedule - 1][b22][b23][1][0:2] + '…', True, (0, 0, 0))
                    window.blit(prename, (475 + print_x, 160 + print_y))
                else:  # 狀態二為變粉紅色+顯示讀書科目名稱
